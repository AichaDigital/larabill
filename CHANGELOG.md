...
- **`larabill:diagnose-price-overlaps` lists article prices that are active at the same time (AID-601).** Read-only, exits `1` when any pair exists, so it can be wired as a **pre-upgrade gate**: a database written before this invariant may already hold overlaps, and those writes will now fail. It deliberately does not repair anything — deciding which of two prices survives is a pricing decision that belongs to the consumer, not to the package.

### Fixed

- **`Article::getPriceFor()` resolves deterministically when a database still holds overlapping prices (AID-601).** It reads a single value out of a set that may hold several rows and did so with no `ORDER BY`, so which price reached the invoice line depended on the index walk — billed money decided by chance. Ordering is now explicit: most recent start wins, `id` breaks the tie (`NULL` sorts lowest in MySQL and SQLite, so a dated row beats an open-start legacy one). This is predictability, not a repair: the fix is removing the duplicate, which `larabill:diagnose-price-overlaps` locates.
...
- **The frozen fiscal snapshot carries the tax rules actually applied (AID-534).** `fiscal_snapshot.tax_rules_applied` was designed in and persisted always `[]` behind a confessional TODO — the frozen evidence claimed a datum it never carried. It now aggregates the items' immutable `taxes_applied` entries (one entry per distinct rule: `source_rate_id`, `name`, `rate`, summed `base` and `amount`, rate descending) — same frozen lines, same instant, so the snapshot cannot drift from the lines by construction. Historical snapshots keep their `[]`; they are frozen evidence and are not rewritten.
...
- **A proforma converts to a final invoice exactly once, also under concurrency (AID-554, D1).** `convertProformaToInvoice()` re-reads the proforma under `lockForUpdate()` before its idempotency check; it used to `refresh()` without a lock, so N concurrent conversions of the same proforma could all pass the check — minting duplicate final invoices on a warm series control, or crashing N−1 callers with a raw `DeadlockException` on the numbering first-use (the nested transaction voids the AID-390 retry). The winner creates; losers now receive the already-created invoice. Public signature intact. Fork-based proof in `tests/Concurrency/ProformaConversionConcurrencyTest.php`, both flavours demonstrated against the unlocked code.
- **A converted invoice can navigate back to its proforma (AID-555, D2).** `invoices.proforma_id` — the column the `proforma()` relation reads — was never written by any real flow; only the inverse mirror (`converted_invoice_id`) was persisted, so the invoice→proforma direction was born broken. `convertProformaToInvoice()` now writes the canonical link, and a **data migration** (`backfill_invoice_proforma_link`) completes it on already-converted pairs from the mirror, without touching consumer-written links. Upgrade: re-run `larabill:install` (publishes only the new migration) + `php artisan migrate`.
- **Converting a proforma carries the customer's accepted figures and the full line, verbatim (AID-557 D4 + AID-556 D3).** The conversion rebuilt each line from 4 fields and re-derived the amounts through `TaxCalculationService` against the LIVE tax catalog — a VAT change between the proforma and its conversion silently altered the final invoice (the persisted fiscal data, not just presentation: same live-vs-frozen family as AID-546/AID-328); `item_type`, `internal_code`, `unit_measure_id`, the service dates, `metadata` and the immutable `taxes_applied` snapshot were silently dropped. The line is now cloned verbatim: frozen amounts and tax breakdown copied from the proforma (ADR-001), never recalculated, and every line field preserved. Normal invoice creation keeps calculating as before.
- **Immutability now protects the lines, not only the header (AID-558, D8).** `Invoice::update()` guarded fiscal content on immutable invoices while `InvoiceItem` had no guard at all — a fiscally frozen invoice could have its lines rewritten freely. `InvoiceItem::update()` now throws on lines of an immutable invoice, same pattern as the header guard (`save()` remains the deliberate internal door). Declared limit, unchanged: raw SQL, query-builder mass updates and external writes stay outside this application-level guarantee (the DB-level half of D8 was cancelled in AID-468).
- **The operation date survives the proforma conversion, and can be set at creation (AID-559, D10).** `invoices.service_date` (RD 1619/2012 art. 6.1.f/i) existed as a column and the PDF already printed it conditionally (AID-442), but no service ever populated it — `createInvoice()`'s explicit whitelist silently dropped it even when passed. The conversion now carries the proforma's declared `service_date` verbatim, and `createInvoice()`/`createProforma()` accept it in `$invoiceData`. **Documented rule:** no source datum → `null` — a date is never invented (the PDF row simply does not print). This is not v7's materialized accrual engine.
- **Concurrent first use of a virgin series no longer crashes callers with a raw `DeadlockException` (AID-570).** The AID-390 numbering retry (`DB::transaction(..., 3)`) runs as a savepoint inside `createInvoice()`/`convertProformaToInvoice()`, and an InnoDB deadlock on the `invoice_series_control` first-use insert aborts the whole outer transaction — the inner retry could never save it, so N−1 concurrent callers died raw (e.g. every January 1st with annual reset). Both outer transactions now retry at the outermost boundary (`attempts: 3`), with the documented contract that their closures stay side-effect-free outside the database. Fork-proven: 6 concurrent `createInvoice()` on a virgin series → 5 raw deadlocks before, strict correlative 1..6 after.
...
### Fixed
- **`InvoiceTemplatesSeeder` no longer seeds the phantom «Plantilla Simple» proforma template, and a guarded data migration removes the row from existing databases (AID-450).** The seeder shipped an ACTIVE proforma template whose `template_path` (`larabill::pdf.invoice.proforma-simple`) never resolved to a blade anywhere in the package history, so any consumer selecting it crashed the render with `View [pdf.invoice.proforma-simple] not found`. The new `remove_phantom_proforma_simple_invoice_template` migration deletes that row from already-seeded databases — unless the consumer published an override blade at that path (the row is functional then, so it is kept). A new registry↔members consistency test iterates every seeded `template_path` and asserts it resolves via `View::exists()`, turning any future phantom entry into an immediate test failure (same spirit as `MigrationOrderConsistencyTest` for `$migrationOrder` ↔ stubs).
