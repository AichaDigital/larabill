...
it('can create a company fiscal config', function () {
    ...
    expect($config->current_eu_sales_amount)->toBe(5000.00);
});
...
it('can check if threshold has been exceeded', function () {
    ...
    $config->update(['current_eu_sales_amount' => 12000.00]);
...
it('can update EU sales amount', function () {
    ...
    $updated = $config->updateEuSales(3000.00);

    expect($updated->current_eu_sales_amount)->toBe(8000.00);
...
it('can get threshold percentage', function () {
    ...
    expect($config->getThresholdPercentage())->toBe(0);
});
...
it('can get remaining threshold amount', function () {
    ...
    $config->update(['current_eu_sales_amount' => 12000.00]);
    expect($config->getRemainingThresholdAmount())->toBe(0);
});
...
it('can get default values from config', function () {
    ...
    expect(CompanyFiscalConfig::getDefaultThreshold())->toBe(15000);
...
it('sets default values on creation', function () {
    ...
    expect($config->current_eu_sales_amount)->toBe(0.00);
});
