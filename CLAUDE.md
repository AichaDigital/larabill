...
- Tag actual: **v6.3.0** (Packagist); `main` limpio; `[Unreleased]` vacío. **PHPStan level 8** (máximo). Cadena reciente:
...
- **Próximo breaking conocido: NINGUNO.** El major de higiene se ejecutó como **v6.0.0** (AID-423) y vació el backlog de deprecated. Desde v6.0.0 rige el **contrato de estabilidad `STABILITY.md`** (en el dist): breaking solo con imperativo de uso cualificado y documentado; todo major auto-upgradeable desde el anterior (maquinaria AID-398); deprecación viva ≥1 major completo antes de retirarse. Un feature/fix que proponga romper superficie sin caso real documentado **contradice el contrato** — parar y confrontar con `STABILITY.md`. `FiscalNumberAllocator` (lado consumer, AID-289) quedó **cableado en `clientes`** (emisión Vía 1). La limpieza post-auditoría (AID-391) se CERRÓ en v6.2.0; AID-442 (fila «Fecha de operación») y AID-450 (plantilla fantasma del seeder) se CERRARON en v6.3.0. Backlog conocido (sin fecha): **AID-444** (conversión proforma→factura: `service_date := invoice_date` por defecto vía flag `proforma.align_service_date_on_conversion`; el flujo de conversión aún no existe — la capa de presentación ya quedó cerrada en v6.3.0) y **AID-328** (`FiscalChangeDetector` ciego a ediciones in-place de las fiscal configs — compara IDs de filas vivas, no snapshots; hallazgo C6 de la revisión adversarial del diseño T8 de Vía 1). Contexto fiscal interno de la pareja 442/444: `docs/2026-07-12-proforma-service-date-fiscal-rationale.md` (gitignorado, local-only).
