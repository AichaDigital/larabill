...
- Tag actual: **v1.0.0** (Packagist); **v2.0.0 en rama `abdelkarim/aid-240-nada-decimal-bd`** (pendiente de PR/merge). Releases recientes:
  - **v2.0.0** (rama) — **AID-240**: cierra la regla dura "nada decimal en BD" para los sistemas paralelos que AID-237 no tocó. Alcance C (solo violaciones reales): (1) `commissions.rate` `decimal(8,4)` → `integer` (DB-only; el cast ya era `FixedDecimalCast:2`, quita el techo de overflow `9999.9999`); (2) `eu_sales_thresholds.total_amount`/`threshold_amount` `decimal(15,2)` → `integer`, y `EuSalesThreshold` pasa de API `float`/euros a `FixedDecimal:2` base-100 (**breaking** → 2.0.0). `breakdown_by_country` migra a céntimos enteros (antes floats euros). Arregla el **bug euro-vs-céntimo** de `checkThreshold` (threshold sembrado en euros vs total en céntimos). `config.destination_vat.default_threshold` `10000.0` → `1000000`. Mutators (`addAmount`/`addSalesAmount`/`addSalesForCountry`) y getters monetarios pasan a `FixedDecimal`; retirados los helpers float del modelo. Factory huérfana saneada. Test MySQL `MoneyColumnTypesTest` asserta `int` (no `decimal`) vía `information_schema`. **Unificación cosmética fuera de scope** (`vat_categories`, `country_vat_rates`, `commissions.*_base100`, `tax_rates` siguen int crudo) → follow-up separado. Ver `docs/upgrade-2.0-eu-sales-thresholds.md`.
  - **v1.0.0** — **AID-237**: migración completa a `FixedDecimal` (lara100 v2). Los 13 atributos monetarios de 8 modelos pasan de `Base100Int` (int) a `FixedDecimalCast:2` (value object inmutable). **Sin cambio de BD** (columnas siguen `integer`, mismos céntimos unscaled). Redondeo de la base imponible de línea corregido de truncación a **HalfUp** (norma EU/España; ±1 céntimo en líneas con cantidad fraccionaria; el IVA ya era HalfUp). Breaking en la API de modelos → **1.0.0**. Ver `docs/ADR-009-fixeddecimal-money-type.md` + `docs/upgrade-1.0-fixeddecimal.md`. Helper de test `cents()`/`fdMoney()`. Sistemas base-100/base-10000 paralelos (`VatCategory`, `CountryVatRate`, `Commission::*_base100`, `TaxRate`) **fuera de scope** (siguen int crudo). Distinción clave: query builder (`->value()`/`->sum()`/`->where()`) devuelve int; acceso a atributo Eloquent devuelve `FixedDecimal`.
  - **v0.11.2** — fix render QR Veri*Factu inline (SVG con declaración XML mal clasificado) + `DomPDFService` que enmascaraba fallos de render devolviendo un mock `TEST-001`; ahora loguea y relanza.
  - **v0.12.0** — empaqueta tres frentes: **AID-136** (intra-EU servicios: breakdown **N2** "no sujeta por reglas de localización" + **IDOtro** para receptor extranjero, regla AEAT 1100); **AID-221** (contrato de retención legal `LegallyRetainable`); **AID-222** (`withTrashed()` en los snapshots fiscales soft-deletables `userTaxProfile()`/`companyFiscalConfig()`). Sube **`lara-verifactu` a `^1.0.0-rc2`** (salto de major, y es un **RC**) por N2+IDOtro.
  - **v0.12.1** — hotfix: `VerifactuAdapter` emitía `serie` como int y rompía `VerifactuInvoice::getSerie()` (`?string`) con `TypeError` en el build XML (ningún registro pasaba end-to-end).
...
- **Dependencias notables:** `lara-verifactu ^1.0.0-rc2` (**RC** — larabill estable depende de un release candidate), `lara-privacy-core ^1.0`, `lara100 ^2.0` (FixedDecimal — AID-237), `lararoi ^0.5`.
