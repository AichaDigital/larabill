...
| `Contracts/` (4) | `@api` | Extension points for consumers and sibling packages (PDF connectors, fiscal verification, tax strategies, the recurring emission hook — AID-836) |
| `Exceptions/` (15) | `@api` | Thrown across the boundary; consumers catch them (AID-836 added `MissingUserTaxProfileException` and `MissingRecurringEmissionHookException`). The count had drifted at 11 and is recounted here: the four missing are `ReverseChargeWithTaxException` (AID-929), `InvalidInvoiceStatusException` (AID-838), `MissingContractPriceException` (AID-956) and `OverlappingArticleOverrideException` (AID-974 — the customer override non-overlap invariant) |
...
- **`@api` (15):** `InvoiceService` (its `convertProformaToInvoice()` gained the additive `series` option in v6.7.0/AID-576 — forwards a per-call fiscal series into the numbering, semver-minor), `InvoiceNumberingService`, `InvoiceSeriesResolver` (AID-307 — the single source of the fiscal series), `InvoiceVerifactuService`, `TaxCalculationService`, `FiscalChangeDetector` (real consumer usage), `RecurringBillingService` (AID-836: emits through `InvoiceService::createInvoice()` inside an atomic per-service boundary; its constructor's `$invoiceNumbering`/`$seriesResolver` params are deprecated-in-prose for removal in the next major, and it optionally receives a consumer-bound `RecurringEmissionHookContract`), `ServiceLifecycleService`, `GroupedPaymentService`, `PricingService`, `EuSalesThresholdService`, `DestinationVatService`, `CommissionCalculationService` (consumer-orchestrated domain), `ArticlePriceService` (AID-601/ADR-012 — the locked write path that guarantees the article price non-overlap invariant), `ArticleOverrideService` (AID-974 — the same for the CUSTOMER override invariant: at most one active override per customer/article pair on any given date, serialised by a lock on the parent `articles` row; create-only and always active by design). (`BillingService`, formerly `@api` + `@deprecated`, was removed in v6.0.0/AID-423.)
