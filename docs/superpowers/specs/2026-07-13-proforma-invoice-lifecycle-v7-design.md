...
- **Protection triggers at FACT registration:** from the first EFFECTIVE accrual-creating fact, plain `cancel()` is forbidden entirely and plain `supersede()` is forbidden. What remains available is **guarded supersession** (audited), which never moves or rewrites append-only records (Codex 82): facts and born obligations stay exactly where they were born, and the supersession records an **audited successor association** — the successor proforma references the predecessor, and effective facts/obligations are resolved THROUGH the supersession link for coverage classification, projection and fulfillment. **Born obligations retain their fiscal origin** (Codex 83): they remain recorded under the predecessor's terms, identity and determination; the association is legal only when the successor is compatible with that recorded truth (same issuer identity; contractual terms consistent with the obligation's amount/currency/nature — validated at supersession time, typed failure otherwise). An incompatible successor (e.g. an issuer identity change) leaves the obligation with the predecessor, to be fulfilled or rectified in THAT context. Guarded supersession is the vehicle for legitimate post-fact commercial modifications (schedule changes before a chargeability date, the audited absorption of a rate difference, §5.1) — the remedies this spec prescribes are reachable exactly when needed (Codex 76).
...
**`createInvoice()`** keeps its call shape, is reimplemented, and is documented NOT behavior-compatible: **two-phase** — (1) a durable phase commits the DRAFT invoice row (the billing aggregate that OWNS the direct facts, Codex 71) together with the registered facts and their obligation rows, so a later failure never erases economic truth (Codex 57); (2) atomic determine (dated resolver) → issue. **The one-operation-date guard applies to direct invoices exactly as to conversions** (§6.6): facts whose accruals do not share one date fail typed (Codex 80). **Direct-aggregate protection (Codex 84):** once effective facts attach in phase one, the DRAFT direct invoice is content-locked (contractual fields immutable through the same guards) and cannot be cancelled while effective facts/obligations exist — the same protection semantics as proformas; a failed phase two is remedied by retrying it or through the unified correction paths, never by editing or discarding the aggregate that owns born obligations. **Facts and nature are REQUIRED explicit input** — larabill never defaults an accrual to the issuance date; a direct sale invoiced at the operation moment declares that fact. Observable changes documented in `UPGRADE-7.0.md`.
...
- **Conversion/issuance:** contractual-copy exactness (incl. `contract_line_total`, adjustment); resolver double throws if invoked; idempotency; per-operation series; `service_date` mapping; differing accrual dates rejected; complete same-date obligation-set precondition; identity matrix at issue; handler-missing fails BEFORE issuing; midnight `fiscal_year == year(invoice_date)`; stalled CONVERTING+PREPARED report.
