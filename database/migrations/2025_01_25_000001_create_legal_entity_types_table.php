...
return new class extends Migration
{
    ...
    public function up(): void
    {
        Schema::create('legal_entity_types', function (Blueprint $table) {
            $table->id();
            $table->string('code', 20)->unique()->comment('Código único de tipo de entidad (PERSONA_FISICA, SOCIEDAD_LIMITADA, etc.)');
            $table->string('name')->comment('Nombre del tipo de entidad jurídica');
            $table->string('name_en')->nullable()->comment('Nombre en inglés');
            $table->string('abbreviation', 10)->nullable()->comment('Abreviatura oficial (SL, SA, etc.)');
            $table->string('country_code', 2)->default('ES')->comment('Código ISO 3166-1 alpha-2 del país');
            $table->text('description')->nullable()->comment('Descripción del tipo de entidad');
            $table->boolean('requires_tax_id')->default(true)->comment('Si requiere identificación fiscal (CIF/NIF)');
            $table->boolean('is_company')->default(false)->comment('Si es una entidad empresarial (vs. persona física)');
